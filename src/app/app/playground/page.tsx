...
function storageKey(email?: string | null) {
  return scopedKey("bsl_playground", scope(email));
}

function loadState(email?: string | null): PlaygroundState {
  const def = ONBOARDING_AGENT;
  const stored = getJSON<Partial<PlaygroundState>>(storageKey(email));
  return {
...
export default function PlaygroundPage() {
  ...
  const [state, setState] = useState<PlaygroundState>(() => loadState(user?.email));
  const [change, setChange] = useState("");
  const [rewriting, setRewriting] = useState(false);

  const activeDef = ONBOARDING_AGENT;
  ...
  function handleSave() {
    ...
    setState(next);
    setJSON(storageKey(user?.email), next);
    ...
    <>
      <div className="page-head">
        <div>
          ...
          <p className="page-sub">Describe how your {activeDef.label} should behave in plain English and it rewrites the prompt for you — no need to touch the system prompt. {activeDef.blurb}</p>
        </div>
      </div>
